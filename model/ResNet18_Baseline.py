...
def build_resnet18(num_classes: int, freeze_backbone: bool = True, device: str | torch.device = "cpu") -> nn.Module:
    try:
...
def build_model(num_classes: int, freeze_backbone: bool = True, device: str | torch.device = "cpu") -> nn.Module:
    return build_resnet18(num_classes=num_classes, freeze_backbone=freeze_backbone, device=device)
